...
async function start() {
  ...
  const listings = await fs.readJson(
    `./detailedListing/detailedListing_${options.collectionId}.json`
  );
  const withArea = listings.map((listing) => {
    const { area } = areaCheck.find((area) =>
      area.check(listing.coordinates)
    ) || { area: "badlands" };
    return { ...listing, area };
  });
